...
function Update_citizens_g() {
    ...
    const handleRegistration =async (data) => {
      const res = await axios.post("http://localhost:4000/recharch_citizens_g", data, {
    ...
    const handelUpdate= async(id)=>{
      const data={'id':id}
      console.log(data)
      const res = await axios.post("http://localhost:4000/Update_citizens_g", data, {
        maxBodyLength: "Infinity",
      })  .then(function (response) {
        console.log(response.data);
        setId(response.data)
  ...
  return (
    <div>
        ...
        <form className="form-width mt-5 p-4 bg-slate-200 form_box3" onSubmit={handleSubmit(handleRegistration)} >
            <div><h1 className="text-3xl font-bold text-gray-700 p-3 uppercase"> Search Citizen</h1></div>
