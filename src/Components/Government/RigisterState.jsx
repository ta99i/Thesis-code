...
function Rigister_State() {
  ...
  const handleRegistration =async (data) => {
    const state=ethers.utils.formatBytes32String('STATES');
    console.log(state)
  const provider = new ethers.providers.Web3Provider(window.ethereum)
  const signer = provider.getSigner()
  const contract = new ethers.Contract(addressOfContract, abi,signer)
  const mint = await contract.grantRole(state,data.state)
  console.log(mint)
  ...
  return (
    ...
    <form className="form-width mt-5 p-4 bg-slate-200  form_box2" onSubmit={handleSubmit(handleRegistration)} >
        <div><h1 className="text-3xl font-bold text-gray-700 p-3">ADD STATE</h1></div>
    <div className="flex justify-around items-center font-bold mt-2">
        <label className="w-1/4">State</label>
        <input className="leading-8 outline-none p-1 pl-2 border-color" name="state" {...register('state')} required/>
      </div>
