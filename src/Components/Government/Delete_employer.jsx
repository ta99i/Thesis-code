...
function Delete_Employer() {
    ...
    const mint = await contract.deleteEmployer(data.employer)
    console.log(mint)
  ...
  return (
    ...
    <form className="form-width mt-5 p-4 bg-slate-200 form_box2 " onSubmit={handleSubmit(handleRegistration)} >
        <div><h1 className="text-3xl font-bold text-gray-700 p-3"> DELETE EMPLOYER</h1></div>
    <div className="flex justify-around items-center font-bold mt-2">
      ...
      <button className="font-bold text-lg mt-5 p-4 bg-gray-700 text-slate-200 w-60 rounded-full border-solid"  >delete</button>
    </form>
    </div>
    </div>
  )
