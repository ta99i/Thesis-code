...
function Update_citizens_p() {
    ...
    const handleRegistration =async (data) => {
      const res = await axios.post("http://localhost:4000/recharch_citizens_t", data, {
    ...
    const handelUpdate= async(id)=>{
      const data={'id':id}
      console.log(data)
      const res = await axios.post("http://localhost:4000/Update_citizens_t", data, {
        maxBodyLength: "Infinity",
      })  .then(function (response) {
        console.log(response.data);
        setId(response.data)
  ...
  return (
    <div>
        ...
        <button className="font-bold text-lg mt-5 p-4  text-slate-200 w-60 rounded-full border-solid" style={{backgroundColor:'#38709c'}}  >search</button>
        </form>
        </div>
        <div>
            {
